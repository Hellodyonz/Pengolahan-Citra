...
terkecil = 1;
for i = 2 : jum
    if Kontur(i, 1) < Kontur(terkecil, 1) || (Kontur(i, 1) == Kontur(terkecil, 1) && Kontur(i, 2) < Kontur(terkecil, 2))
        terkecil = i;
    end
end
...
for i = 1 : jum
    ...
    Piksel(indeks).sudut = sudut(Kontur(terkecil, :), Kontur(i, :));
    Piksel(indeks).jarak = jarak(Kontur(terkecil, :), Kontur(i, :));
end
jum_piksel = indeks;
...
stat = ((p2.x - p1.x) * (p3.y - p1.y) - (p3.x - p1.x) * (p2.y - p1.y)) > 0;
end
